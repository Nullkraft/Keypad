...
|| |
|| |  "Stated" version notes:
|| |    This version uses the new "Stated" version of the event listener function. The difference
|| |    is that the key state is passed into this event listener, avoiding a lot of crawling around
|| |    in internal data structures of the library, and allowing easier multiple key detection.
|| #
*/
#include <Keypad.h>
#include <ctype.h>
...
void setup() {
    ...
    ltrpad.addStatedEventListener(keypadEvent_ltr);  // Add an event listener.
    ltrpad.setHoldTime(500);                   // Default is 1000mS
    numpad.addStatedEventListener(keypadEvent_num);  // Add an event listener.
...
void keypadEvent_ltr(KeypadEvent key, KeyState kpadState) {
    // in here when in alpha mode.
    swOnState( key, kpadState );
} // end ltrs keypad events

void keypadEvent_num( KeypadEvent key, KeyState kpadState ) {
    // in here when using number keypad
    swOnState( key, kpadState );
} // end numbers keypad events


void swOnState( KeypadEvent key, KeyState kpadState ) {
    switch( kpadState ) {
        case PRESSED:
